...
nwis_sites_by_state <- map(c("CO", "WY", "UT", "KS", "NM"), # c("Colorado", "Wyoming", "Utah", "Kansas", "New Mexico")
                           ~{
                             ...
                             if (.x != "CO") {
                               ...
                               # Same filter as above, but first remove the old sites, and
                               # then filter out the gages that are under 1500 square kilometers
                               final_nwis_call <- nwis_call %>% 
                                 filter(!(site_no %in% old_usgs_site_names)) %>% 
                                 mutate(drain_area_km = drain_area_va *  2.58999) %>%
                                 filter(drain_area_km <= 1500) %>% 
...
#
# write_rds(data_gap_gages, here("data", "data_gap_gages.rds"))

# Only keep gages with data during or after 1980:
# down from 2525 to 303 sites, and drops all of the original sites
...
filtered_cdwr_sites <- cdwr_sites %>% 
  filter(!(abbrev %in% old_cdwr_site_names),
         ...
         !(usgsSiteId %in% data_filter_nwis_sites$site_no), # Prevent duplicate sites from nwis site filterd data
...
write_rds(streamcat_watersheds, here("data", "streamcat_watershed_data", "watersheds_streamcat.RDS"))
...
reference_watersheds <- streamcat_watersheds %>% 
  mutate(area = sf::st_area(streamcat_watersheds)) %>%
...
watersheds <- read_rds(here("data", "watershed_polygons.RDS"))
...
single_test <- fetch_flowlines(
  site_no = watersheds$site_no[21],
  STUSPS = watersheds$STUSPS[21],
  geometry = watersheds$geometry[21],
...
all_successful <- list()
# update chunks here
chunks <- chunks[31:length(chunks)]
for (chunk_idx in seq_along(chunks)) {
...
flowline_names <- map(.x=names(all_flowlines_list), ~sub("^.*_", "", .x)) %>% 
  unlist()

subset_watersheds <- watershed_polygons %>% 
  filter(site_no %in% flowline_names)
test_site <- watersheds_div$site_no[1]
...
```{r}
mapview(filter(watersheds_div, site_no == "06614800")) + mapview(all_flowlines_list[["CO_06614800"]])
```
