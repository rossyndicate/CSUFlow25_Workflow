...
Next, filter to only sites with drainage areas of 1500 sqkm based on USGS WS size:
...
Subset to gages with data since 2000:
...
gage_meta <- bind_rows(gage_meta_1,gage_meta_2)
...
nwis_sites <- nwis_sites_data_record %>% bind_rows(cdwr_sites) %>%
...
# Some USGS gages have been handed off to CDWR (and, many USGS gages are listed in CDWR)
# Here I'm removing those dupes, keeping the CDWR "version"
dupe_test <- nwis_sites %>%
...
nwis_sites <- nwis_sites %>%
  # keeping all CDWR sites based on email from CWCB about seasonal sites?
